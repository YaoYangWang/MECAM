...
class IntraModalityCL(nn.Module):
    ...
    def _get_positive_mask_by_label(self, labels):
        # 获取样本数
        batch_size = labels.size(0)
        
        # 初始化全 0 掩码
        mask = torch.zeros((batch_size, batch_size), device=labels.device)
        
        # 遍历样本，将同类样本设置为正样本（即掩码值为 1）
    ...
    def forward(self, video_features, text_features, labels):
        ...
        # 生成正样本掩码（根据标签）
        positive_mask = self._get_positive_mask_by_label(labels)
    
        # 获取负样本（非正样本）
        negatives_vid = logits_clstr_vid * (1 - positive_mask)
        negatives_txt = logits_clstr_txt * (1 - positive_mask)
        ...
        # 拼接正样本和负样本
        vid_logits = torch.cat([self.negative_w * negatives_vid, self.negative_w * positives_vid], dim=1)
        txt_logits = torch.cat([self.negative_w * negatives_txt, self.negative_w * positives_txt], dim=1)
